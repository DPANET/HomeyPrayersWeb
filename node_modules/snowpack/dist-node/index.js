...
async function command$1(commandOptions) {
  ...
  for (const [outLoc, file] of Object.entries(allFilesToResolveImports)) {
    ...
    const resolvedCode = await transformFileImports(file, spec => {
      ...
      const resolvedImportPath = removeLeadingSlash(path.normalize(resolvedImportUrl)); // We treat ".proxy.js" files special: we need to make sure that they exist on disk
      ...
      if (isAbsoluteUrlPath) {
        return path.relative(path.dirname(outLoc), path.resolve(buildDirectoryLoc, resolvedImportPath)).replace(/\\/g, '/'); // replace Windows backslashes at the end, after resolution
